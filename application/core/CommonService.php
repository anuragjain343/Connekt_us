...
class CommonService extends REST_Controller{
    ...
    public function check_service_auth(){
        ...
        if(array_key_exists ('auth_token', $header )){
   ...
   //send push notifications
    public function send_push_notification($userName,$token_arr, $title, $body, $reference_id, $type,$userType,$profileImage,$reference_type,$notifications_by_id){ 
        if(empty($token_arr)){
            return false;
        }

        //prepare notification message array
        $notif_msg = array('title'=>$title, 'body'=> $body, 'reference_id'=>$reference_id,'profile_image'=>$profileImage ,'type'=> $type, 'click_action'=>$userType, 'sound'=>'default','username'=>$userName,'reference_type'=>$reference_type,'notifications_by_id'=>$notifications_by_id);

         $this->notify_log = FCPATH.'notification_log.txt';
            log_event($token_arr,$this->notify_log);

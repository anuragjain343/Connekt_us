...
class ContactList_model extends CI_Model {
    ...
    var $column_order = array('`contact`.`contactUsId`','`contact`.`name`','`contact`.`email`','`contact`.`message`','`contact`.`subject`','`contact`.`status`'); //set
    var $column_search = array('name','email','`contact`.`subject`','message','status'); //set column field database for datatable searchable
    var $order = array('contact.contactUsId' => 'DESC');  // default order
    var $where = '';
    ...
    private function posts_get_query(){
       ...
       $this->db->select($sel_fields);
        $this->db->from('`contactUs` `contact`');
        $i = 0;

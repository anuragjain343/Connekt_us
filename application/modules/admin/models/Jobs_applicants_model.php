...
class Jobs_applicants_model extends CI_Model {

    //var $table , $column_order, $column_search , $order =  '';
    var $table = JOBS;

    var $column_order = array('u.userId','u.fullName','u','profileImage','u.email','ja.applied_by_user_id','ja.job_application_status','ja.job_type'); //set

    var $column_search = array('u.fullName','u.email'); //set column field database for datatable searchable
    var $order = array('ja.jobApplicantId' => 'DESC');  // default order
    var $where = '';
    ...
    private function posts_get_query(){
       ...
       $this->db->select('`u`.`userId`,`u`.`fullName`,`u`.`email`,`ja`.`applied_by_user_id`,`ja`.`job_application_status`,`ja`.`job_type`,`ja`.`jobApplicantId`,(case 
               ...
               END ) as profileImage
        ');
       $this->db->join('`users` `u`','`u`.`userId` = `ja`.`applied_by_user_id`');
       //$this->db->join('`users` `u` ','`u`.`userId` = `sp`.`specializationId`');
       //$this->db->join('`interviews` `interview`','`interview`.`request_id` = `request`.`requestId`');
        $this->db->from('`job_applicants` `ja`');
        $i = 0;
        ...
        foreach ($this->column_search as $emp) // loop column 
        {
            ...
            if(!empty($_POST['job_status'])){
                ...
                $this->db->where('job_application_status !=',0); 
            }

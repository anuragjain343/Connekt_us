...
class Jobs_model extends CI_Model {
    ...
    var $column_search = array('jt.jobTitleName','sp.specializationName','j.job_location','u.businessName'); //set column field database for datatable searchable
    ...
    private function posts_get_query(){
       ...
       // pr($sel_fields);
       $this->db->select('`j`.`jobId`,`j`.`job_type`,`j`.`job_title_id`,`j`.`job_location`,`j`.`industry`,`j`.`status`,`jt`.`jobTitleId`,`jt`.`jobTitleName`,`sp`.`specializationId`,`sp`.`specializationName`,`u`.`fullName`,`u`.`businessName`');
       ...
       $this->db->join('`users` `u`','`u`.`userId` = `j`.`posted_by_user_id`');
        ...
        foreach ($this->column_search as $emp) // loop column 
        {
            ...
            if(!empty($_POST['userId'])){
              $this->db->where('posted_by_user_id',$_POST['userId']); 
            }
    ...
    function jobDetail($jobId,$job_type){
      ...
      $res->applicants    = $this->common_model->get_total_count(JOB_APPLICANTS, $where=array('job_id'=>$jobId));   
     ...
     function getJobTitle_data($search){
        ...
        $result[$key]->count = $this->jobTitleCount($value->jobTitleId);
        }
    ...
    function getSpecilization_data($search){
         
        $result = $this->db->select('specializationId,specializationName')->order_by('specializationName', 'ASC')->or_where(array('userType'=>'both'))->get(SPECIALIZATIONS)->result();

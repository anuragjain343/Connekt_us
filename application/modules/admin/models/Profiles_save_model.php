...
class Profiles_save_model extends CI_Model {

    //var $table , $column_order, $column_search , $order =  '';
    var $table = USERS;

    var $column_order = array('u.userId','u.profileImage','u.fullName','u.businessName','val.valueName','str.strengthName','u.email','u.status','sp.saveprofileId'); //set

    var $column_search = array('u.businessName','u.fullName'); //set column field database for datatable searchable
    var $order = array('sp.saveprofileId' => 'DESC');  // default order
    var $where = '';
    ...
    $this->db->select('`u`.`userId`,`u`.`status`,`u`.`fullName`,`u`.`email`,(case 
       ...
       $this->db->join('`save_profiles` `sp`','`sp`.`profile_id` = `u`.`userId`');
       ...
       //$this->db->join('`interviews` `interview`','`interview`.`request_id` = `request`.`requestId`');
        ...
        foreach ($this->column_search as $emp) // loop column 
        {
            ...
            $this->db->where('save_by_user_id',$_POST['user_id']); 

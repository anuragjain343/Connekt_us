...
class Service extends CommonService{
    ...
    function registration_post(){
        ...
        $this->form_validation->set_rules('userType', 'User Type', 'trim|required|min_length[2]|max_length[50]|callback__alpha_spaces_check_username'); 
        ...
        if($this->form_validation->run() == FALSE){
            ...
            $authtoken = $this->service_model->generate_token();
            $data = array();
            $set = array('fullName','email','businessName','phone','userType','deviceToken','deviceType');
            ...
            if(is_array($result)){
                ...
                switch ($result['regType']){
                    case "NR":
                    $response = array('status'=>SUCCESS,'message'=>ResponseMessages::getStatusCodeMessage(110),'userDetail'=>$result['returnData'],'is_profile_complite'=>0,'is_billing_complete'=>0);
                    break;
                    case "AE":
    ...
    public function resetPassword_post(){
        ...
        if($is_updated){
            ...
            $subject = 'ConnektUs-Reset Password';

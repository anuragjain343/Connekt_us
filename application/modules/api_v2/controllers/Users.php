...
class Users extends CommonService{
    ...
    function getJobTitle_post(){
        ...
        // $where = array('userType'=>$this->authData->userType,'status'=>1);
        //,$offset,$limit
        $result = $this->Users_model->getJobTitle_data($search);
    ...
    function _alpha_spaces_check_username($string){
        if(alpha_spaces($string)){
            return true;
        }
        else{
            $this->form_validation->set_message('_alpha_spaces_check_username','Please do not use symbols or numbers in Full Name field');
            return FALSE;
        }
    }
    ...
    function updateBusinessProfile_post(){
       ...
       // $this->form_validation->set_rules('bio', 'Bio', 'trim|required|min_length[2]|max_length[200]');
        ...
        $set = array('address','city','state','country', 'bio','latitude', 'longitude','description','linkedIn_profile_link');
        ...
        $jobTitle = ucwords($this->post('job_title'));
        //$insert_data['jobTitle_id'] = $this->post('job_title');

            if(!empty($jobTitle)){

                $where    = array('jobTitleName'=>trim($jobTitle));

                $jobtitle = $this->common_model->getsingle(JOB_TITLES,$where,$fld =NULL,$order_by = '',$order = '');

                if(!empty($jobtitle)){
                    $insert_data['jobTitle_id'] = $jobtitle->jobTitleId;
                }else{

                    $data1['jobTitleName']  = trim($jobTitle);
                    $data1['userType']      = 'both';
                    $data1['type']          = 1;
                    $data1['addedById']     = $this->authData->userId;
                    $title_id               = $this->common_model->insertData(JOB_TITLES, $data1);
                    $insert_data['jobTitle_id']  = $title_id;
                } 
        ...
        $specializationName = ucwords($this->post('area_of_specialization'));
            ...
            if(!empty($specializationName)){
                ...
                if(!empty($jobspecializationName)){
                    ...
                    $speciality_data['specialization_id'] = $jobspecializationName->specializationId;
                    ...
                    $speciality_data['specialization_id']  = $title_id;
                } 
        ...
        $speciality_data = array('user_id'=>$current_user_id, 'specialization_id'=>$speciality_data['specialization_id']);
        if($exist){
        ...
        if(!empty($insert_images)) {
         ...
         $wherelink  = array('user_id'=>$current_user_id);   
         $userdata =  $this->common_model->getsingle(USER_META, $wherelink,'linkedIn_profile_link');
        $response = array('status'=>SUCCESS, 'message'=>ResponseMessages::getStatusCodeMessage(125),'company_logo'=>$dataImage,'linkedIn_profile_link'=>$userdata->linkedIn_profile_link);
        $this->response($response);
    }
      //$this->load->model('Users_model'); //load user model
    ...
    function updateBasicInfo_post(){
        ...
        //$this->form_validation->set_rules('linkedIn_profile_link', 'linkedIn profile link', 'trim|required');
        ...
        $set = array('address','city','state','country', 'bio','latitude', 'longitude','linkedIn_profile_link');
        foreach ($set as $key => $val) {
            $post= $this->post($val);
            if(!empty($post))
                $insert_data[$val] = $post;
        ...
        $specializationName = ucwords($this->post('area_of_specialization'));
            ...
            if(!empty($specializationName)){
                ...
                if(!empty($jobspecializationName)){
                    ...
                    $insert_data['specializationId'] = $jobspecializationName->specializationId;
                    ...
                    $datasp['addedById']               = $this->authData->userId;
                    $title_id                         = $this->common_model->insertData(SPECIALIZATIONS,$datasp);
                    $insert_data['specializationId']  = $title_id;
                } 

            }

        $this->common_model->insertData(USER_SPECIALIZATION_MAPPING, array('user_id'=>$current_user_id, 'specialization_id'=>$insert_data['specializationId']));

       $wherePofile = array('user_id'=>$this->authData->userId);   
        $isProfilecomplite  = $this->common_model->getsingle(USER_EXPERIENCE,$wherePofile,$fld =NULL,$order_by = '',$order = '');
        $is_profile_complite = 0;

        if(empty($isProfilecomplite)){
            $is_profile_complite ='0';
        }else{
            $is_profile_complite ='1';
        }

        $whereBilling = array('user_id'=>$this->authData->userId);   
        $isbillingcomplite  = $this->common_model->getsingle(USER_BLLLING_INFO,$whereBilling,$fld =NULL,$order_by = '',$order = '');
        $is_billing_complete = 0;

        if(empty($isbillingcomplite)){
            $is_billing_complete ='0';
        }else{
            $is_billing_complete ='1';
        }
    
        $response = array('status'=>SUCCESS, 'message'=>ResponseMessages::getStatusCodeMessage(125),'is_profile_complite'=>$is_profile_complite,'is_billing_complete'=>$is_billing_complete);
    ...
    function updateResume_post(){
        ...
        $response = array('status'=>SUCCESS, 'message'=>ResponseMessages::getStatusCodeMessage(143),'is_profile_complite'=>$is_profile_complite,'is_billing_complete'=>$is_billing_complete);
        $this->response($response);
    ...
    function updateExperience_post(){
        ...
        if(isset($exp_data['current_job_title']) && !empty($exp_data['current_job_title'])){
            $jobProfileCompleted = 1;
                 $jobTitleId='';
            //if(!empty($exp_data['current_job_title'])){
                $where    = array('jobTitleName'=>trim($exp_data['current_job_title']));
                $jobtitle = $this->common_model->getsingle(JOB_TITLES,$where,$fld =NULL,$order_by = '',$order = '');
                if(!empty($jobtitle)){
                   $exp_data['current_job_title'] = $jobtitle->jobTitleId;
                }else{

                    $data1['jobTitleName']  = ucwords(trim($exp_data['current_job_title']));
                    $data1['userType']      = 'both';
                    $data1['type']          = 1;
                    $data1['addedById']     = $this->authData->userId;
                    $title_id              = $this->common_model->insertData(JOB_TITLES, $data1);
                    $exp_data['current_job_title']            = $title_id;
                } 
            //}
        }


           $specializationName = ucwords($this->post('next_speciality'));
       
           if(isset($specializationName) && !empty($specializationName)){
                ...
                if(!empty($jobspecializationName)){
                    ...
                    $exp_data['next_speciality'] = $jobspecializationName->specializationId;

                }else{

                    $data12['specializationName']      = trim($specializationName);
                    $data12['userType']                = 'both';
                    $data12['type']                    = 1;
                    $data12['addedById']               = $this->authData->userId;
                    $title_id                         = $this->common_model->insertData(SPECIALIZATIONS,$data12);
                    $exp_data['next_speciality']      = $title_id;
                } 
        ...
        if($is_exist){
            ...
            foreach($decode as $value){
                ...
                //if(!empty($value->previous_job_title)){
                    //$privious_exp['previous_job_title_id']='';
                    $jobProfileCompleted = 1;
                    $jobTitleId='';
                    if(!empty($value->previous_job_title)){
                        $where43    = array('jobTitleName'=>trim($value->previous_job_title));
                        $jobtitle = $this->common_model->getsingle(JOB_TITLES,$where43,$fld =NULL,$order_by = '',$order = '');
                        if(!empty($jobtitle)){

                            $privious_exp['previous_job_title_id'] = $jobtitle->jobTitleId;

                        }else{

                            $data16['jobTitleName']  = ucwords(trim($value->previous_job_title));
                            $data16['userType']      = 'both';
                            $data16['type']          = 1;
                            $data16['addedById']     = $this->authData->userId;
                            $title_id              = $this->common_model->insertData(JOB_TITLES,$data16);
                            $privious_exp['previous_job_title_id']            = $title_id;
                        } 
                    }
              //  }
            ...
            foreach($decode as $value){
               ...
               // if(!empty($value->previous_job_title)){
                    ...
                    $jobTitleId='';
                    if(!empty($value->previous_job_title)){
                        $where12    = array('jobTitleName'=>trim($value->previous_job_title));
                        $jobtitle = $this->common_model->getsingle(JOB_TITLES,$where12,$fld =NULL,$order_by = '',$order = '');
                        if(!empty($jobtitle)){
                            ...
                            $data13['jobTitleName']  = ucwords(trim($value->previous_job_title));
                            $data13['userType']      = 'both';
                            $data13['type']          = 1;
                            $data13['addedById']     = $this->authData->userId;
                            $title_id              = $this->common_model->insertData(JOB_TITLES, $data13);
                            $privious_exp[' previous_job_title_id']          = $title_id;
                        } 
                    }
                //}
        ...
        $response = array('status'=>SUCCESS, 'message'=>ResponseMessages::getStatusCodeMessage(125),'previous_role'=>!empty($privious_exp)?$privious_exp:[],'is_profile_complite'=>$is_profile_complite,'is_billing_complete'=>$is_billing_complete);
        $this->response($response);
    ...
    function addFavourites_post(){
        ...
        if($this->form_validation->run() == FALSE){
            ...
            if(!empty($user_info)){
                ...
                //– Good news! UserName has added your profile to their favourites!
                $body_send = 'Good news! ' .$this->authData->fullName.' has added your profile to their favourites!';  //body to be sent with current notification
                $body_save = 'Good news! [UNAME] has added your profile to their favourites!'; //body to be saved in DB
                $notif_type = 'user_favourites';

                if($user_info->isNotify == 1){
                    $notif_msg = $this->send_push_notification($this->authData->fullName,$registrationIds, $title, $body_send, $favourite_for, $notif_type,$userType,$profileImage,'',$current_user_id);
        ...
        if($this->form_validation->run() == FALSE){
            ...
            if(!empty($user_info)){
                ...
                if(!empty($this->authData->profileImage)){
                 ...
                 //Guess what!? UserName has recommended you to other employers!
                if($this->authData->userType =='business'){  

                 $body_send = 'Guess what!? ' .$this->authData->fullName.' has recommended you to other employers!';  
                 $body_save = 'Guess what!? [UNAME] has recommended you to other employers!'; 

                }else{

                    $body_send = 'Guess what!? ' .$this->authData->fullName.' has recommended you to other job seekers!';  
                   $body_save = 'Guess what!? [UNAME] has recommended you to other job seekers!'; 
                } 
              //body to be sent with current notification
                ...
                if($user_info->isNotify == 1){
                    $notif_msg = $this->send_push_notification($this->authData->fullName,$registrationIds, $title, $body_send, $recommend_for, $notif_type,$userType,$profileImage,'',$current_user_id);
    ...
    function getIndivisualSearchList_v1_post(){
        $this->check_service_auth();
        // log_event(json_encode($_POST));
        ...
        $search_list = $this->Users_model->get_indivisual_search_list_v1($search, $limit, $offset,$address,$city,$state,$country);
    ...
    function getIndivisualSearchList2new_post(){
        $this->check_service_auth();
        // log_event(json_encode($_POST));
        //$otherBusiness1     = $this->post('otherBusiness');
        //$searchId           = $this->post('searchId');
        if($this->post('pagination') == 1){
        ...
        //log_event($_POST['country']);    
        //log_event('param:::::: '.json_encode($search));
        $search_list = $this->Users_model->get_indivisual_search_listnew($search, $limit, $offset,$address,$city,$state,$country,$searchId,$isfilter);
        $search_count=0;
        //if($this->post('isFilter') == 1 AND empty($otherBusiness1)){
          if($this->post('isFilter') == 1){
        $search_count11 = $this->Users_model->exectMatch($search,$limit, $offset,$address,$city,$state,$country,$searchId, false,$isfilter);

        $query = $this->db->query($search_count11);
        $res   = $query->result();
      ...
      $search_city = $city; //current user city(in case no filter is applied)
       ...
       /* $otherBusiness='';
        if(!empty($search_list) AND empty($otherBusiness1)){
           $otherBusiness= FALSE; 
        }else if(empty($search_list) AND !empty($otherBusiness1)){
           $otherBusiness= FALSE; 
        }
        else{
            $otherBusiness= TRUE; 
        ...
        $response = array('status' => SUCCESS,'searchList'=>$search_list,'recordFound'=>$recordFound,'searchCount'=>$search_count); //success msg
        //}else{
            //$response = array('status' => SUCCESS,'searchList'=>$search_list,'recordFound'=>$recordFound,'otherBusiness'=>FALSE,'searchCount'=>$search_count);
        //}
        ...
        //log_event($_POST['country']);    
        //log_event('param:::::: '.json_encode($search));
        $search_list = $this->Users_model->get_indivisual_search_list($search, $limit, $offset,$address,$city,$state,$country,$otherBusiness1,$searchId);
        $search_count=0;
        if($this->post('isFilter') == 1 AND empty($otherBusiness1)){

            $search_count = $this->Users_model->get_indivisual_search_list_count($search, $limit, $offset,$address,$city,$state,$country,$otherBusiness1,$searchId);
        }
      ...
      $search_city = $city; //current user city(in case no filter is applied)
        ...
        if($this->post('isFilter') == 1){
            ...
            $response = array('status' => SUCCESS,'searchList'=>$search_list,'recordFound'=>$recordFound,'otherBusiness'=>$otherBusiness,'searchCount'=>$search_count); //success msg
        }else{
            $response = array('status' => SUCCESS,'searchList'=>$search_list,'recordFound'=>$recordFound,'otherBusiness'=>FALSE,'searchCount'=>$search_count);
        }
    ...
    function getBusinessSearchList_v2_post(){
       //log_event(json_encode($_POST));
        //check for auth
        $this->check_service_auth();
        ...
        $search_list = $this->Users_model->get_business_search_list($search, $limit, $offset,$address,$city,$state,$country,$otherBusiness1,$searchId);
        //search records for city only- This is required becuse at app end they need to show message based on records exits or not
        //log_event($this->db->last_query());
        $search_city = $city; //current user city(in case no filter is applied)
        ...
        $response = array('status' =>SUCCESS,'searchList'=>$search_list,'recordFound'=>$recordFound,'otherBusiness'=>$otherBusiness); //success msg
        $this->response($response);
    ...
    function getPublicProfile_get(){
        ...
        $is_save_profile = $this->common_model->getsingle(SAVE_PROFILES,array('profile_id'=>$user_id ,'save_by_user_id'=>$this->authData->userId));
        if(!empty($is_save_profile)){
         $is_save_profile =1;   
        }else{
        $is_save_profile =0; 
        }

        $is_active_job = $this->common_model->get_total_count(JOBS,array('posted_by_user_id'=>$user_id ,'status'=>1));

        if(!empty($is_active_job)){
         $is_activeJob = $is_active_job;   
        }else{
        $is_activeJob = 0; 
        }
    ...
    function profileUpdate_post(){
        ...
        $this->form_validation->set_rules('fullName', 'Name', 'trim|required|min_length[2]|max_length[50]|callback__alpha_spaces_check_username');
        ...
        if($this->form_validation->run() == FALSE){
            ...
            $set = array('address','city','state','country','latitude', 'longitude','bio', 'description','linkedIn_profile_link');
            ...
            $jobTitle = ucwords($this->input->post('job_title'));
             $jobTitleId='';
            if(!empty($jobTitle)){
                $where    = array('jobTitleName'=>trim($jobTitle));
                $jobtitle = $this->common_model->getsingle(JOB_TITLES,$where,$fld =NULL,$order_by = '',$order = '');
            ...
            $speciality_data = array('specialization_id'=>$insert_data['specializationId']);
...
*/
    ...
    function updateIndivisualProfile_post(){
        ...
        if($this->form_validation->run() == FALSE){
            ...
            $data = array();
            $set = array('address','city','state','country','latitude', 'longitude','bio','linkedIn_profile_link');
            foreach ($set as $key => $val) {
                $post= $this->post($val);
                $data[$val] = (isset($post) && !empty($post)) ? $post :''; 
            }
    ...
    function badge_count_unread_views_get(){
        ...
        if($userType == 'individual'){
            ...
            $get = array('profile_view','user_recommends','user_favourites','user_review');
            $total_count = $this->Users_model->get_total_counts($get,$where);
        }
    ...
    function badge_total_count_unread_views_get(){
        $this->check_service_auth();
        $userId  = $this->authData->userId;
        $userType  = $this->authData->userType;
        $where = array('isViewed'=>0,'notification_for'=>$userId);
        if($userType == 'individual'){
            $get = array('profile_view','user_recommends','user_favourites','shortlisted_job','interview_request','rejected_job','job_offered_reject','job_offered','new_job');
            $total_count = $this->Users_model->get_total_counts($get,$where);
        }else{
            $get = array('profile_view','user_recommends','user_favourites','user_review','accept_interview_request','apply_job','decline_interview_request');
            $total_count = $this->Users_model->get_total_counts($get,$where);
        }
        $res = $this->Users_model->get_count_viewed_data(NOTIFICATIONS,$get,$userId);
        $response = array('status'=>SUCCESS,'message'=>'count','total'=>$total_count,'data'=>$res);
    ...
    function update_notification_v2_post(){
        $this->check_service_auth();
        $id =  $this->authData->userId;
        $notiy =  $this->post('notify_type');
        $notify_by =  $this->post('notify_by');
        ...
        $read_all =  $this->post('read_all');

        if($read_all==1){

           $whereIn = 'new_job,shortlisted_job,apply_job,interview_request,job_offered,job_offered_reject,accept_interview_request,decline_interview_request';
           $whereIn = explode(',',trim($whereIn));
           $res =  $this->Users_model->updateNofication_readall(NOTIFICATIONS,$whereIn);

        }else{

            $read_all==0;

            if($reference_id==0 OR $notify_by==0){

                $where = array('notification_for'=>$id,'notification_type'=>$notiy);

            }else{
                
                $where = array('notification_by'=>$notify_by,'notification_for'=>$id,'notification_type'=>$notiy,'reference_id'=> $reference_id);
            }
    ...
    function getUserVerifiedStatus_get(){//get current status of user email verification.
        ...
        // save user profile business side.in this api we have to save job seekar profiles

    function saveProfileBusinessSide_post(){

        $this->check_service_auth();
        $this->form_validation->set_rules('profile_id', 'User Profie id', 'trim|required');
        $userId =$this->authData->userId;
        if($this->form_validation->run() == FALSE){
            $response = array('status' => FAIL, 'message' => preg_replace("/[\\n\\r]+/", " ",strip_tags(validation_errors())));
            $this->response($response);die();
            ...
            $whereProfile     = array('userId'=>$this->post('profile_id'));
            $profiles         = $this->common_model->getsingle(USERS,$whereProfile,$fld =NULL,$order_by = '',$order = '');
            if(empty($profiles)){
                $response = array('status' => FAIL, 'message'=>'Somthing went wong.');
                $this->response($response);die(); 
            }
            $whereProfileId  = array('profile_id'=>$this->post('profile_id'),'save_by_user_id'=>$userId);
            $is_profiles   = $this->common_model->getsingle(SAVE_PROFILES,$whereProfileId,$fld =NULL,$order_by = '',$order = '');

            if($is_profiles){

                 $response = array('status' => FAIL, 'message'=>'Profile already saved.');
                $this->response($response);die(); 
            }
            ...
            $data['crd']                   = datetime();  
            $data['upd']                   = datetime();  
            $saveProfiles                  = $this->common_model->insertData(SAVE_PROFILES,$data);
            if($saveProfiles){
    $response = array('status' => SUCCESS, 'message'=>'Profile has been saved successfully.');
                $this->response($response);die(); 
            }
    ...
    function saveProfileIndividualSide_post(){
    ...
    //REMOVE PROFILE WHICH IS SAVED BY USERS
    function removeSavedProfile_post(){
        ...
        if($this->form_validation->run() == FALSE){
            ...
            $whereprofile  = array('profile_id'=>$this->post('profile_id'),'save_by_user_id'=>$userId);
            $is_profiles   = $this->common_model->getsingle(SAVE_PROFILES,$whereprofile,$fld =NULL,$order_by = '',$order = '');

            if(empty($is_profiles)){

                $response = array('status' => FAIL, 'message'=>'Somthing went wong.');
                $this->response($response);die(); 
            }

            $is_delete   = $this->common_model->deleteData(SAVE_PROFILES,$whereprofile);

            if($is_delete){
            $response = array('status' => SUCCESS, 'message'=>'Saved Profile has been removed successfully');
              $this->response($response);die(); 
            }
    ...
    //END OF THE FUNCTION
        ...
        //SAVE PROFILE LIST WHICH IS SAVED BY USERS
    function savedProfileList_post(){
        ...
        $jobList    = $this->Users_model->get_save_profile_list($limit,$offset);
    ...
    function notificationList_get(){
        ...
        $response = $this->Users_model->notificationList($userId,$offset,$limit);
        if(is_array($response)){
            $response = array('status'=>SUCCESS,'message'=>'OK','data'=>$response,'currentDateTime'=>datetime());
        }else{
            $response = array('status'=>FAIL,'message'=>'Not found','notificationList'=>array());
        }
        $this->response($response);
    }// 

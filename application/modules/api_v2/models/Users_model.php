...
class Users_model extends CI_Model {
    ...
    function get_business_list($where){
        ...
        $defaultImg = base_url().DEFAULT_USER;
            ...
            if($userType->userType == 'business'){
                    $this->db->select('u.userId,u.fullName,u.businessName,u.email,u.userType,u.authToken,u.status,u.crd,u.deviceToken,u.phone,u.deviceType,u.isProfile,u.isNotify,COALESCE(job.jobTitleName,"") as jobTitleName,COALESCE(CEIL(AVG(r.rating)/0.5) * 0.5, "") as rating,COALESCE(aos.specializationName,"") as specializationName,um.linkedIn_profile_link,
                ...
                $this->db->select('u.userId,u.fullName,u.businessName,u.email,u.phone,u.userType,u.authToken,u.status,u.crd,u.deviceToken,u.deviceType,u.isProfile,u.isNotify,COALESCE(aos.specializationName,"") as specializationName,COALESCE(job.jobTitleName,"") as jobTitleName,um.linkedIn_profile_link,
                   ...
                   END ) as profileImage
                        ');
                    $this->db->from(USERS .' as u');
                     $this->db->join(USER_META. ' as um', "um.user_id = u.userId","left"); //to get user 
                    $this->db->join(USER_SPECIALIZATION_MAPPING. ' as usm', "usm.user_id = u.userId","left"); //to get area of specialization detail
            ...
            $res = $result->row();
            $res->is_profile_complite = $is_profile_complite; 
            return $res;
    }
    ...
    function get_business_user($where){
        ...
        $this->db->select('COALESCE(um.address,"") as address,um.linkedIn_profile_link,COALESCE(um.description,"") as description,COALESCE(usr.phone,"") as phone,COALESCE(um.city,"") as city,COALESCE(um.state,"") as state,COALESCE(um.country,"") as country,COALESCE(um.latitude,"") as latitude,COALESCE(um.longitude,"") as longitude,COALESCE(um.bio,"") as bio,
   ...
   function get_indivisual_basic_info($where){
        ...
        $this->db->select('COALESCE(um.address,"") as address,um.linkedIn_profile_link,COALESCE(um.description,"") as description,COALESCE(um.city,"") as city,COALESCE(um.state,"") as state,COALESCE(um.country,"") as country,COALESCE(um.latitude,"") as latitude,COALESCE(um.longitude,"") as longitude,COALESCE(um.bio,"") as bio,COALESCE(aos.specializationId,"") as specializationId,COALESCE(aos.specializationName,"") as specializationName,
    ...
    function get_my_favourites($user_id, $userType, $limit, $offset, $check_status=true){
        ...
        $this->db->select('fav.*, u.fullName,um.address,u.businessName,NOW() as today_datetime,COALESCE(aos.specializationName,"") as specializationName,COALESCE(job.jobTitleName,"") as jobTitleName,(case 
                    when( u.profileImage = "" OR u.profileImage IS NULL) 
                    THEN "'.$defaultImg.'"
                    ELSE
                    concat("'.$user_image.'",u.profileImage) 
    ...
    function get_indivisual_search_list_v1($search, $limit, $offset,$address,$city,$state,$country){
        //pr($search);
        $search_loc ='';
    ...
    //get indivisual user list according to filteres
    function get_indivisual_search_listnew($search, $limit, $offset,$address,$city,$state,$country,$searchId,$isfilter){
       // pr($search);
        $exectMatch = $this->exectMatch($search,$limit, $offset,$address,$city,$state,$country,$searchId, false,$isfilter);
        $fornotin   = $this->exectMatch($search,$limit, $offset,$address,$city,$state,$country,$searchId, true,$isfilter);
        $query_notIn = $this->db->query($fornotin);
        $res_notIn    = $query_notIn->result(); 
        $user_ids = array();
        //pr($fornotin);
        if(!empty($res_notIn)){
        foreach ($res_notIn as $key => $value) {
           $user_ids[] = $value->userId;
        }
        }

        $relatedMatch = $this->relatedMatch($search,$limit, $offset,$address,$city,$state,$country,$searchId,$user_ids,$isfilter);
        //pr($relatedMatch);
        if($offset==0){
            $offset=0;
        }
        $filterQuery = "SELECT * FROM ((SELECT * FROM (".$exectMatch.") as exact_search) UNION ( SELECT * FROM  (".$relatedMatch.") as related_search)) as filtertmptbl LIMIT ".$offset.",".$limit;
        //echo $filterQuery; exit();
        $query = $this->db->query($filterQuery);
        $res    = $query->result(); 
        return $res; 
        
    }

    function exectMatch($search,$limit, $offset,$address,$city,$state,$country,$searchId, $isForNotIN,$isfilter){
       ...
       if(!empty($search['location'])){
        ...
        if(!empty($isfilter)){
        $match ='1 as exectMatch';
        }else{
             $match ='2 as exectMatch';
        }
        ...
        $selectColumns = "";
        if(!empty($search['job_title'])){
            $selectColumns = $isForNotIN ? 'usr.userId':'usr.userId,'.$match.',usr.status,usr.isActive,exp.currentExperience,exp.totalExperience,usr.upd as userTab,um.upd as Metta,usr.fullName,COALESCE(aos.specializationName,"") as specializationName,COALESCE(exp.current_company,"") as company,COALESCE(job.jobTitleName,"") as jobTitleName,COALESCE(um.address,"") as address,
            COALESCE(um.latitude, "") as latitude,COALESCE(um.longitude, "") as longitude,
            (case 
                    when( usr.profileImage = "" OR usr.profileImage IS NULL) 
                    THEN "'.$defaultImg.'"
      ...
      //this query will work to add privious experience with same job title seperatly. to get result of user with privious work experience
       $this->db->join("(SELECT user_id as UID, previous_job_title_id as JOBTITLEID,SUM(`experience`) as `previous_role_exp1` FROM user_previous_role GROUP BY least(user_id, previous_job_title_id) , greatest(user_id, previous_job_title_id)) as prev_user_exp_t1", "`prev_user_exp_t1`.`UID` = `usr`.`userId`","left"); 
        ...
        //$this->db->group_start();
       /* if(!empty($search)){
            if($otherBusiness1==TRUE AND !empty($searchId)){

            $search['job_title'] = $search['expectedSalaryTo'] = $search['experienceTo'] = $search['experienceFrom']= $search['expectedSalaryFrom']= $search['state']= $search['country']= $search['city']= $search['location']= $search['rating']=  $search['speciality_id']= $search['employementType']= $search['strength']= $search['availability']= $search['company']='';
            }
        }*/
       ...
       /* if(!isset($search['expectedSalaryFrom']) AND !empty($search['expectedSalaryTo']) AND $search['expectedSalaryTo'] != 99999999){
        ...
        if(!empty($searchId)){
          $searchId1 =  explode(",",$searchId);
          $this->db->where_not_in('usr.userId',$searchId1);
        }
        $this->db->where(array('usr.status'=>1,'usr.userType'=>'individual','usr.isActive'=>1, 'usr.isVerified'=>1, 'usr.jobProfileCompleted'=> 1));
        $this->db->group_by('usr.userId');
        $this->db->order_by('usr.upd', 'DESC');
    ...
    function relatedMatch($search,$limit, $offset,$address,$city,$state,$country,$searchId,$fornotin,$isfilter){
       ...
       if(!empty($search['location'])){
        ...
        $user_image = base_url().USER_IMAGE;
        if(!empty($search['job_title'])){
        $this->db->select('usr.userId,'.$match.',usr.status,usr.isActive,exp.currentExperience,exp.totalExperience,usr.upd as userTab,um.upd as Metta,usr.fullName,COALESCE(aos.specializationName,"") as specializationName,COALESCE(exp.current_company,"") as company,COALESCE(job.jobTitleName,"") as jobTitleName,COALESCE(um.address,"") as address,
            COALESCE(um.latitude, "") as latitude,COALESCE(um.longitude, "") as longitude,
            (case 
                    when( usr.profileImage = "" OR usr.profileImage IS NULL) 
                    THEN "'.$defaultImg.'"
      ...
      //this query will work to add privious experience with same job title seperatly. to get result of user with privious work experience
       ...
       /* if(!empty($search)){
        ...
        !empty($search['speciality_id']) ? $this->db->or_where(array('usm.specialization_id'=>$search['speciality_id'])) : "";
        
        !empty($search['employementType']) ? $this->db->or_where(array('exp.employementType'=>$search['employementType'])) : "";
       
        !empty($search['strength']) ? $this->db->or_where(array('ustr.strength_id'=>$search['strength'])) : ""; 
       
        !empty($search['availability']) ? $this->db->or_where(array('exp.next_availability'=>$search['availability'])) : ""; 
       
        !empty($search['value']) ? $this->db->or_where(array('uv.value_id'=>$search['value'])) : "";
         //$this->db->group_end();
       ...
       /* if(!isset($search['expectedSalaryFrom']) AND !empty($search['expectedSalaryTo']) AND $search['expectedSalaryTo'] != 99999999){
        ...
        $this->db->group_end(); //master group end
        if(!empty($fornotin)){
        $this->db->where_not_in('userId',$fornotin);
        }
        if(!empty($search_loc)){
            $this->db->group_start();
     ...
     function get_indivisual_search_list($search, $limit, $offset,$address,$city,$state,$country,$otherBusiness1,$searchId){
       // pr($search);
        $search_loc ='';
       if(!empty($search['location'])){
        $search_loc = $search['location'];
    ...
    function get_indivisual_search_list_count($search, $limit, $offset,$address,$city,$state,$country,$otherBusiness1,$searchId){
       // pr($search);
        $search_loc ='';
       if(!empty($search['location'])){
        $search_loc = $search['location'];
      ...
      //this query will work to add privious experience with same job title seperatly. to get result of user with privious work experience
       $this->db->join("(SELECT user_id as UID, previous_job_title_id as JOBTITLEID,SUM(`experience`) as `previous_role_exp1` FROM user_previous_role GROUP BY least(user_id, previous_job_title_id) , greatest(user_id, previous_job_title_id)) as prev_user_exp_t1", "`prev_user_exp_t1`.`UID` = `usr`.`userId`","left"); 
        ...
        if(!empty($search)){
            ...
            $search['job_title'] = $search['expectedSalaryTo'] = $search['experienceTo'] = $search['experienceFrom']= $search['expectedSalaryFrom']= $search['state']= $search['country']= $search['city']= $search['location']= $search['rating']=  $search['speciality_id']= $search['employementType']= $search['strength']= $search['company']='';
            }
        }
        !empty($search['speciality_id']) ? $this->db->where(array('usm.specialization_id'=>$search['speciality_id'])) : "";
    ...
    function get_business_search_list_v2($search, $limit, $offset,$address,$city,$state,$country,$otherBusiness,$searchId){
        ...
        if($otherBusiness==TRUE AND !empty($searchId)){
           $search['jobTitle']= $search['speciality_id']=$search['rating']=$search['company']='';
        }
    ...
    function get_my_business_profile($where){
        ...
        $this->db->select('COALESCE(u.fullName, "") as fullName,COALESCE(u.phone, "") as phone,COALESCE(um.description, "") as description,COALESCE(u.email, "") as email,COALESCE(u.businessName, "") as businessName,COALESCE(u.userType, "") as userType,COALESCE(job.jobTitleName, "") as jobTitleName,COALESCE(um.address, "") as address,COALESCE(um.city, "") as city,COALESCE(um.state,"") as state,COALESCE(um.country,"") as country, COALESCE(um.latitude, "") as latitude,COALESCE(um.longitude, "") as longitude,COALESCE(um.bio, "") as bio,
            COALESCE(aos.specializationName, "") as specializationName,um.linkedIn_profile_link,COALESCE(CEIL(AVG(r.rating)/0.5) * 0.5, "") as rating,
            (case 
                    when( um.company_logo = "" OR um.company_logo IS NULL) 
                    THEN "'.$default_logo.'"
                    ELSE
    ...
    function getComapny($limit, $offset) {
        ...
        $this->db->where(array('userType' => 'business','isProfile'=> 1));
   ...
   function resend_mail($where,$data){
    ...
    //SAVE PROFILE LIST
    function get_save_profile_list($limit,$offset){

        $defaultImg = base_url().DEFAULT_USER;
        $user_image = base_url().USER_IMAGE;  
        $userId     = $this->authData->userId;
        $this->db->select('u.userId,u.fullName,sj.crd,um.address,u.businessName,(case 
                when( u.profileImage = "" OR u.profileImage IS NULL) 
                THEN "'.$defaultImg.'"
                ELSE
                concat("'.$user_image.'",u.profileImage) 
            END ) as profileImage,ue.current_job_title,jt.jobTitleName');

        $this->db->from(USERS .' as u');
        $this->db->join(SAVE_PROFILES. ' as sj', "sj.profile_id = u.userId AND sj.save_by_user_id ='".$userId."' ","left");
        $this->db->join(USER_META. ' as um', "user_id = u.userId","left"); 
        ...
        $this->db->where('sj.save_by_user_id',$userId);
        $this->db->limit($limit, $offset);
        $result = $this->db->get(); 
        $res = $result->result();
        return $res;
        ...
        // Notification list 
    function notificationList($userId,$limit,$offset){
        $defaultImg = base_url().DEFAULT_USER;
        $user_image = base_url().USER_IMAGE;  
        $this->db->select('n.*,u.userId,u.fullName,(case 
                when( u.profileImage = "" OR u.profileImage IS NULL) 
                THEN "'.$defaultImg.'"
                ELSE
                concat("'.$user_image.'",u.profileImage) 
        ...
        $this->db->join(USERS.' as u','u.userId = n.notification_by','left');
        $this->db->where('n.notification_for',$userId);
        ...
        if($query->num_rows() > 0){
            ...
            foreach ($rows as $key => $value){
             ...
             $value->notification_message = json_decode($value->notification_message);
            }
            return $rows;
        }else{
            return false ;
    ...
    function updateNofication_readall($table,$whereIn){

        $data=array('isViewed'=>1);
        $this->db->set($data);
        $this->db->where_in('notification_type',$whereIn);
        $this->db->where('notification_for',$this->authData->userId);
